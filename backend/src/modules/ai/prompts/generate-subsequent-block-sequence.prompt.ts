...
/**
 * Prompt for generating subsequent block sequence (block_sequence_counter > 0)
 * Generates: inform block with keyMisconceptions + practice block
 */
export const generateSubsequentBlockSequencePrompt = ({
  ...
  const wrongAnswersText = wrongAnswers && wrongAnswers.length > 0
...
**CRITICAL: The keyMisconceptions array MUST contain between 2-4 items. Do NOT include more than 4 misconceptions.**
...
Expected format:
{
  ...
  "informBlock": {
    ...
    "keyMisconceptions": [
      ...
      "Third misconception with **highlighted concepts**"
    ],
    "summary": "One sentence overview with **bold key terms** that frames the topic"
  }
}

IMPORTANT: The keyMisconceptions array must have 2-4 items maximum. In this example, there are 3 key misconceptions.`;
};
